...
export const verifyOtp = async (req: Request, res: Response) => {
  ...
  const isOtpValid = checkOtp(value.otp.value, otp);
...
export const resendOtp = async (req: Request, res: Response) => {
  ...
  if (!value?.otp.noOfAttempts) {
    res.status(403).json({
      message: "No of Attempts Not Valid",
    });
    return;
  }

  if (value?.otp.noOfAttempts! > 5) {
    res.status(403).json({
